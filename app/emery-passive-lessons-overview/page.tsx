...
export default async function EmeryPassiveLessonsOverviewPage({
  searchParams,
}: {
  ...
  const staffDashboard =
    data && !dashboardError ? (
      <MatrixStaffDashboard
        ...
        basePath="/emery-passive-lessons-overview"
        resolveStudentHref={(id) => {
          const q = cohortOnlyParams(sp.cohort);
          q.set("student", id);
          const qs = q.toString();
          return qs ? `/emery-passive-lessons-overview?${qs}` : `/emery-passive-lessons-overview`;
        }}
      />
    ) : null;
